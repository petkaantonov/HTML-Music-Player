...
import ChannelMixer, { ChannelCount } from "audio/ChannelMixer";
...
import { allocResampler, freeResampler } from "audio/pool";
import getCodecName from "audio/sniffer";
import DatabaseUsingBackend from "common/DatabaseUsingBackend";
import JobProcessor, { ALL_JOBS_COMPLETE_EVENT, JOB_COMPLETE_EVENT } from "common/JobProcessor";
import * as io from "io-ts";
import parseAcoustId, { AcoustIdResponse, CoverArtResponse, ParsedAcoustIdResult } from "metadata/acoustId";
import parseMp3Metadata from "mp3_metadata";
import SearchBackend from "search/SearchBackend";
import { MAX_BUFFER_LENGTH_SECONDS as BUFFER_DURATION } from "src/audio/buffering";
import FileReferenceDeletedError from "src/errors/FileReferenceDeletedError";
import {
    ...
    TrackInfo,
    TrackMetadata,
} from "src/metadata/MetadataManagerFrontend";
...
import { RemoveFirst } from "src/types/helpers";
import { _, ajaxGet, delay, queryString, toCorsUrl, trackInfoFromFileName } from "src/util";
import { CancellationToken, CancellationTokenOpts } from "src/utils/CancellationToken";
...
> {
    ...
    constructor(wasm: WebAssemblyWrapper, database: TagDatabase, searchBackend: SearchBackend) {
        super("metadata", database, {
            setRating({ trackUid, rating }) {
                if (!this.canUseDatabase()) return;
                void this.database.updateRating(trackUid, rating);
            },

            setSkipCounter({ trackUid, counter, lastPlayed }) {
                if (!this.canUseDatabase()) return;
                void this.database.updateSkipCounter(trackUid, counter, lastPlayed);
            },

            setPlaythroughCounter({ trackUid, counter, lastPlayed }) {
                if (!this.canUseDatabase()) return;
                void this.database.updatePlaythroughCounter(trackUid, counter, lastPlayed);
            },
