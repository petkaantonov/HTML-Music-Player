...
(function($) {
	...
	$.jPlayer.prototype = {
		_init: function() {
			...
			this.jPlayerController = jQuery.proxy( this.jPlayerController, this );
			...
			$.extend(this.config, {
				...
				swf: "http://tuohiniemi.fi/~runeli/petka/nettipeli/es/Jplayerbalance.swf", /*this.config.swfPath + ((this.config.swfPath != "" && this.config.swfPath.slice(-1) != "/") ? "/" : "") */
			...
			$.extend(this.config, {
				usingFlash: !(this.config.nativeSupport),
			...
			var eventsForHtmlAudio = {
				stopUpdates: function(){
				self.config.audio.removeEventListener( "timeupdate", self.jPlayerController );
				jQuery.data( self.element[0], "stopUpdates", true )
				},
				setFile: function(e, mp3, ogg, silent) {
					...
					self.config.audio.autobuffer = true; // In case not preloading, but used a jPlayer("load")
					self.config.audio.preload = false; // In case not preloading, but used a jPlayer("load")
					self.config.audio.src = self.config.diag.src;
					self.config.audio.load();
					self.config.isFileSet = true;
						if( mp3 || ogg ) {
						self.jPlayerOnProgressChange(0, 0, 0, 0, 0, 0, 0);
						}
					self.config.audio.removeEventListener( "timeupdate", self.jPlayerController );
						if( !silent ) {
						self.config.audio.addEventListener( "timeupdate", self.jPlayerController );
						}
				...
				load: function(e) {
				jQuery.data( self.config.audio, "changed", false );
				...
				play: function(e) {
					if(self.config.isFileSet) {
						...
						jQuery.data( self.element[0], "stopUpdates", false );
						element.trigger("jPlayer.setButtons", true);
						self.config.audio.removeEventListener( "timeupdate", self.jPlayerController );
						self.config.audio.addEventListener( "timeupdate", self.jPlayerController );
					}
			...
			if(this.config.usingFlash) {
				...
				var lol = 0;
				this.config.audio.addEventListener("error",
					function(){
					
						if( self.config.diag.src ) {
						self.config.audio.removeEventListener( "timeupdate", self.jPlayerController );
						self.jPlayerOnErrorCustom();
						}
					}, false
				);
				...
				this.config.audio.addEventListener("ended", function() {
					self.config.audio.removeEventListener( "timeupdate", self.jPlayerController );
						if( jQuery.data( self.element[0], "stopUpdates" ) === true ) {
						return;
						}
					jQuery.data( self.element[0], "changed", true );
		...
		stopUpdates: function() {
			this.element.trigger("jPlayer.stopUpdates");
		},
		setFileSilent: function(mp3, ogg) {
			this.element.trigger("jPlayer.setFile", [mp3, ogg, true]);
		},
		...
		jPlayerOnProgressChange: function(lp, ppr, ppa, pt, tt, bl, bt) { // Called from Flash / HTML5 interval
			...
			this.config.diag.bytesLoaded = bl;
			this.config.diag.bytesTotal = bt;
			...
			this.onProgressChangeCustom(lp, ppr, ppa, pt, tt, bl, bt);
			this._forceUpdate();
		},
		jPlayerController: function(override) { // The HTML5 interval function.
			var pt = 0, tt = 0, ppa = 0, lp = 0, ppr = 0, bl = 0, bt = 0;
			...
			this.config.audio.removeEventListener( "timeupdate", this.jPlayerController );
			}
			
			
		this.jPlayerOnProgressChange(lp, ppr, ppa, pt, tt, bl, bt);
		},
		volumeMin: function() {
			this.volume(0);
		},
