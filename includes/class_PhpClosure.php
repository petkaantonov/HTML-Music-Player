...
class PhpClosure {
  ...
  var $_outputFile = "compiler-min.js";
  var $_srcs = array();
   ...
   */
  function write() {
  $content = $this -> _compile();
  
  	if( !$content["code"] || !strlen( $content["code"] ) ) {
  	echo $content["result"] . "\n\n";
  	return false;
  	}
  return @file_put_contents( $this -> _outputFile, $content["code"] );
  }
