...
import AudioPlayerFrontend, { ChannelData } from "audio/AudioPlayerFrontend";
import { ChannelCount } from "metadata/MetadataManagerFrontend";
...
type Deps = SelectDeps<"page" | "audioManager" | "workerWrapper">;
interface Opts {
    maxFrequency: number;
    minFrequency: number;
    bufferSize: number;
...
export default class AudioVisualizerFrontend extends WorkerFrontend<AudioVisualizerResult> {
    ...
    constructor(opts: Opts, deps: Deps) {
        super("visualizer", deps.workerWrapper);
        this.page = deps.page;
