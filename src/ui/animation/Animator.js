...
const makePathEasing = function(path) {
    ...
    return function(current, total) {
        ...
        for (let i = 0; i < path.length; ++i) {
            const start = path[i].startX();
            const end = path[i].endX();

            if (start <= progress && progress <= end) {
                const progressWithin = (progress - start) / (path[i].endX() - start);
                path[i].yAt(progressWithin);
                return;
            }
        }
    };
...
Animator.prototype._gotAnimationFrame = function(now) {
    ...
    for (let i = 0; i < this._properties.length; ++i) {
        ...
        const propertyIsFinished = property.tween($elem, elapsed);
...
Animator.prototype.animationEnd = function() {
    return new Promise((resolve) => {
        this.once(`animationEnd`, (_, cancelled) => resolve(cancelled));
    });
};

Animator.LINEAR = function(a, b) {
