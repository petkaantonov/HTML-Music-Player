import { StoredKVValues } from "Application";
//@ts-ignore
import EventEmitter from "jsevents";
import { DatabaseClosedEmitterTrait, DatabaseEventsMap } from "platform/DatabaseClosedEmitterTrait";
import { decode, EventEmitterInterface, typedKeys } from "types/helpers";
import { applyStoreSpec, iDbPromisify } from "utils/indexedDbUtil";

import { DatabaseClosedError } from "./platform";
...
class KeyValueDatabase extends EventEmitter {
    ...
    private _uiLogRef: (...args: string[]) => void;
    ...
    async setAll(preferenceKeyValuePairs: { key: string; value: string }[]) {
        if (this.isClosedAndEmit()) return;
        const db = await this.db;
        const store = db
