...
export const normalizeQuery = function(value) {
    value = `${value}`;
    const ret = unicode.removeDiacritics(value.
...
export let float32BE;
export let float32BEString;
export let float32LE;
export let int32BEString;
export let uint32BEString;
export let float64BE;
export let float64LE;
...
export function synchInt32(bytes, offset) {
...
export function int16BE(bytes, offset) {
    return ((bytes.charCodeAt(offset + 0) & 0xFF) << 8) |
           (bytes.charCodeAt(offset + 1) & 0xFF);
}

export function int32LE(bytes, offset) {
...
export function int16LE(bytes, offset) {
