...
import { MIN_SUSTAINED_AUDIO_SECONDS, SUSTAINED_BUFFERED_AUDIO_RATIO, WEB_AUDIO_BLOCK_SIZE } from "buffering";
import * as io from "io-ts";
import { ChannelCount, FileReference, Track } from "metadata/MetadataManagerFrontend";
...
import SourceDescriptor, { AudioBufferSourceNodeExt } from "SourceDescriptor";
import { decode, EventEmitterInterface } from "types/helpers";
import ApplicationPreferencesBindingContext from "ui/ApplicationPreferencesBindingContext";
import EffectPreferencesBindingContext, { AudioPlayerEffects } from "ui/EffectPreferencesBindingContext";
...
interface StateModificationAction {
    type: "suspend" | "resume";
    promise: Promise<void>;
}

interface BufferFilledResult {
    type: "bufferFilled";
...
export default class AudioPlayerFrontend extends WorkerFrontend<AudioPlayerResult> {
    ...
    private _preloadingTrack: Track | null = null;
    ...
    constructor(deps: Deps) {
        super("audio", deps.workerWrapper);
    ...
    loadTrack(track: Track, _isUserInitiatedSkip: boolean, initialProgress: number = 0, resumeAfterLoad: boolean) {
