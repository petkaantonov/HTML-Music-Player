...
import { PreferenceArray } from "shared/src/preferences";
...
const ASSET_CACHE = ASSET_PREFIX + revision.replace(/[^a-z0-9]+/gi, `-`);
...
const isCorsUrl = function (url: string) {
    return new URL(url).origin !== location.origin;
};

const isUnnecessary = function (url: string) {
    return url.indexOf(`images`) >= 0 && ricon.test(url) && url.indexOf(`apple-touch-icon-180x180`) === -1;
};

async function fetchAssetToCache(url: string, cache: Cache) {
    let retries = 0;
    while (retries < 5) {
        try {
...
async function cacheAssets() {
    ...
    for (const asset of assets) {
        const url = new URL(asset).toString();
...
async function cacheResponse(request: Request, response: Response, cacheName: string) {
    const cache = await caches.open(cacheName);
    return cache.put(request, response);
}

async function getMatchedAsset(request: Request) {
    ...
    if (cacheName) {
        await cacheResponse(fetchRequest, response.clone(), cacheName);
    }
    return response;
}
...
async function savePreferences(preferences: PreferenceArray) {
