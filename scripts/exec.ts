...
const exec = (cmd: string, { stdin = "pipe" as const, responseTimeout = undefined }: Options = {}): Promise<string> => {
    let selfTerm = false;
    const name = cmd.split(" ")[0];
    const logFile = `${tmpdir}/exec.log`;
        ...
        ee.on(`exit`, async (code, signal) => {
            ...
            if (code === 0) {
                ...
                reject(
                    Object.assign(
                        new Error(
                            `\`${cmd}\` timed out with no response within ${(responseTimeout / 1000) | 0} seconds`
                        ),
                        out
                    )
                );
