...
const removeDiacritics = (function () {
    ...
    const initialize = function () {
        ...
        for (let i = 0; i < diacriticMappingList.length; i += 2) {
            const base = diacriticMappingList[i]!;
            const chars = diacriticMappingList[i + 1]!;

            let encodedBaseValue = 0;
    ...
    const diacriticReplacer = function (theChar: string) {
        const encodedBaseValue = mapping[theChar.charCodeAt(0) & 0xffff]!;
        ...
        if (encodedBaseValue <= 0xff) {
            ...
            return String.fromCharCode((encodedBaseValue & 0xff00) >> 8, encodedBaseValue & 0xff);
        } else {
            return String.fromCharCode(
                (encodedBaseValue & 0xff0000) >> 16,
...
export const unicode = {
    ...
    alphaNumericFilteringPattern: new RegExp(`[^ ${characterCategories.numbers}${characterCategories.letters}]+`, `g`),
    separateWordsPattern: new RegExp(`[${characterCategories.wordSeparator}]+`, `g`),
    removeDiacritics,
};

export const joinAbbreviations = function (str: string) {
    const words = str.split(` `).filter(word => word.length > 0);

    if (words.length > 1) {
        let singleCharWordStart = -1;
        for (let i = 0; i < words.length; ++i) {
            ...
            if (word.length === 1) {
                ...
                const newWord = words.slice(singleCharWordStart, i).join(``);
        ...
        if (singleCharWordStart >= 0) {
            const newWord = words.slice(singleCharWordStart).join(``);
...
export const normalizeQuery = function (value: string) {
    const ret = unicode
        .removeDiacritics(
            value.replace(unicode.separateWordsPattern, ` `).replace(unicode.alphaNumericFilteringPattern, ``)
        )
        .replace(/ {2,/g, ` `)
        .toLowerCase()
        .trim();
