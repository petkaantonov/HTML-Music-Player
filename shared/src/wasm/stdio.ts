...
printFSizeMap[102] = printFSizeMap[70] = printFSizeMap[101] = printFSizeMap[69] = printFSizeMap[97] = printFSizeMap[65] = printFSizeMap[103] = printFSizeMap[71] = f64;

printFSizeMap[99] = printFSizeMap[100] = printFSizeMap[105] = printFSizeMap[111] = printFSizeMap[120] = printFSizeMap[88] = printFSizeMap[117] = printFSizeMap[115] = printFSizeMap[112] = i32;
...
const stdio = [null, (msg: string) => console.log(msg), (msg: string) => console.error(msg)];
const rspecifier = /%([-+#0 ]?[0-9*]?)(\.[0-9*])?(hh|h|j|z|t|L|ll|l)?([%sgGnpaAeEfFuxXodic])/g;

export interface Stdio {
    fprintf: (a: number, b: number, c: number) => number;
            ...
            if (specifier === 37) {
                ...
                if (m3Length === 0) {
                    ...
                    if (cc === 104) {
                        ...
                        ret += signed ? `${wasm.i64AsDouble(alignedOffset)}` : `${wasm.u64AsDouble(alignedOffset)}`;
...
export default function createCStdio(wasm: WebAssemblyWrapper): Stdio {
    const fprintf = function (filePtr: number, formatStringPtr: number, argvPtr: number) {
    ...
    const printf = function (formatStringPtr: number, argvPtr: number) {
        const str = format(wasm, formatStringPtr, argvPtr);
        console.log(str);
        return str.length;
    };
