...
export default class TagDatabase {
    ...
    async getLoudnessAnalyzerStateForTrack(trackUid: ArrayBuffer) {
        ...
        const ret = await iDbPromisify(store.get(trackUid));
        if (ret) {
    ...
    async fileReferenceToFileView(fileReference: FileReference): Promise<FileView> {
        if (fileReference instanceof File) {
            return new FileView(fileReference);
        } else if (fileReference instanceof ArrayBuffer) {
            const file = await this.fileByFileReference(fileReference);
