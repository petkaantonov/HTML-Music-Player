...
export function merge<T>(comparer: Comparer<T>, a: T[], b: T[]) {
    let k = 0;
    let i = 0;
    let j = 0;
    const aLen = a.length;
...
function removeSortedLinear<T>(comparer: Comparer<T>, array: T[], value: T, length: number) {
...
export function remove<T>(comparer: Comparer<T>, array: T[], value: T) {
    const { length } = array;
...
function insertSortedLinear<T>(comparer: Comparer<T>, array: T[], value: T, length: number) {
    for (let i = 0; i < length; ++i) {
...
export function insert<T>(comparer: (a: T, b: T) => number, array: T[], value: T) {
    const { length } = array;
    if (length === 0) {
        array.push(value);
        return true;
